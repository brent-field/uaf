...
class TestBaselineMergingFractions:
    ...
    y-ranges.  The current ``_same_baseline()`` function compares each new
    raw line against only the *previous* raw line's bbox (``prev_bbox``
    drift) rather than the visual line group's bounding-box envelope.
    ...
    def _setup(self) -> None:
        ...
        bbox = self.level_addition_block["bbox"]
        self.line_tops, self.line_lefts = _compute_line_positions(
    ...
    def test_line_tops_count_matches_visual_lines(self) -> None:
        """line_tops tuple length should equal the visual line count."""
        assert self.line_tops is not None, (
            "Level Addition block should have line_tops (multi-line block)"
        )
        assert len(self.line_tops) == len(self.visual_lines), (
            f"line_tops has {len(self.line_tops)} entries but "
            f"visual_lines has {len(self.visual_lines)}"
        )

    def test_line_gaps_not_smaller_than_font_size(self) -> None:
        """Gap between consecutive line_tops should be >= 60% of 10pt.
        ...
        tight_gaps: list[str] = []
        for i in range(len(self.line_tops) - 1):
            gap = self.line_tops[i + 1] - self.line_tops[i]
            if gap < min_gap:
                tight_gaps.append(
                    f"  gap {i}-{i + 1}: {gap:.1f}pt "
                    f"(tops: {self.line_tops[i]:.1f}, "
                    f"{self.line_tops[i + 1]:.1f})"
                )

        assert not tight_gaps, (
            f"Line gaps smaller than {min_gap:.1f}pt found — "
